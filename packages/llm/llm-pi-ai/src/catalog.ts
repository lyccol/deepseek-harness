...
/**
 * Whether one configuration layer decides any compat switch. Both the
 * model-level rejection and the route-level one ask this of their own layer,
 * so the set of switches lives in one place rather than in each condition.
 * @param compat - the switches one layer declared, when any.
 * @returns whether that layer decides at least one switch.
 */
function definesCompatSwitch(compat: PiAiCompatProfile | undefined): boolean {
  return compat?.thinkingFormat !== undefined
...
export interface RouteCatalogRequest {
  ...
  /** Compat switches for every `openai-completions` model on the route; entries override per field. */
  compat?: PiAiCompatProfile
...
function resolveModelCompat(
  ...
  if (api !== 'openai-completions') {
    if (definesCompatSwitch(entry.compat)) {
      invalid(provider, `model "${entry.id}" sets compat switches, but its api is "${api}";`
        + ' they exist only on openai-completions')
    }
    return {}
  }
...
export function resolveRouteModels(request: RouteCatalogRequest): RouteCatalog {
  ...
  const routeCompatDefined = definesCompatSwitch(request.compat)
  ...
  if (routeCompatDefined && !models.some(model => model.api === 'openai-completions')) {
    invalid(provider, 'sets compat switches, but no model on the route speaks openai-completions;'
      + ' they exist only on that protocol')
  }
